...
int main() {
    ...
    AIGD::ConfigParser configParser;
    AIGD::AppConfig    appConfig = configParser.parse("config.json");

    std::cout << "  [Config] Kalman Q=" << appConfig.kalmanFilter.Q
              << "  R=" << appConfig.kalmanFilter.R << "\n";
    std::cout << "  [Config] Grid " << appConfig.mapData.width
              << "x" << appConfig.mapData.height << "\n";
    std::cout << "  [Config] Priority=" << appConfig.navigationPriority << "\n";

    // Decode the Base64 map layout and initialise the occupancy grid
    const std::string flatMap = AIGD::ConfigParser::decodeBase64(
        appConfig.mapData.gridBase64);

    OccupancyGrid missionGrid(appConfig.mapData.width, appConfig.mapData.height);

    // Mark any '#' cells in the decoded map as pre-known obstacles
    // (three Bayesian updates push probability past the 0.7 occupied threshold)
    for (int gy = 0; gy < appConfig.mapData.height; ++gy) {
        for (int gx = 0; gx < appConfig.mapData.width; ++gx) {
            const int idx = gy * appConfig.mapData.width + gx;
    ...
    // Convert navigation priority string to enum
    AIGD::MissionPriority navPriority = AIGD::MissionPriority::BALANCED;
    if (appConfig.navigationPriority == "STEALTH") {
        navPriority = AIGD::MissionPriority::STEALTH;
    } else if (appConfig.navigationPriority == "AGGRESSIVE") {
        navPriority = AIGD::MissionPriority::AGGRESSIVE;
    }

    // Build mission configuration from parsed values
    AIGD::MissionConfig missionConfig;
    ...
    return (missionSuccess && reportOk) ? 0 : 1;
}
